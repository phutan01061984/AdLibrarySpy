...
async function scrapeAdLibraryPage(brandName, brandId, options = {}) {
  ...
  try {
    ...
    // Strategy: Facebook Ad Library renders "Library ID: XXXXX" as text, not as link hrefs.
    // We find all text nodes containing Library IDs and extract surrounding content.
    const ads = await page.evaluate((brandId, brandName) => {
      const results = [];
      const seenIds = new Set();
      const bodyText = document.body.innerText || '';

      // Split body text into ad blocks
      // Each ad block starts with status (Active/Inactive) and contains a Library ID
      ...
      for (const block of adBlocks) {
        ...
        // Active/Inactive — usually the first word before Library ID
        const isActive = !block.match(/^Inactive/i) && !block.includes('Inactive\nLibrary ID');

        // Date range: "Jan 19, 2026 - Feb 28, 2026" or "Started running on..."
        ...
        // Extract advertiser/brand name (line after "Sponsored" usually)
        ...
        // Extract copy text — filter out UI labels
        ...
        const contentLines = lines.filter(l => {
          ...
          if (/^\w{3}\s+\d{1,2},\s*\d{4}/.test(l)) return false; // date lines
