...
async function hashFromUrl(url) {
  try {
    // Facebook CDN images may block direct server-side requests.
    // Try direct fetch first with proper headers.
    let fetchUrl = url;
    const isFacebookCdn = url.includes('fbcdn') || url.includes('facebook') || 
                          url.includes('scontent') || url.includes('external');
    ...
    if (!response.ok) {
      console.error(`[Hasher] HTTP ${response.status} for ${url.substring(0, 80)}...`);
      return null;
    }

    const contentType = response.headers.get('content-type') || '';
    if (!contentType.includes('image') && !contentType.includes('octet-stream')) {
      console.error(`[Hasher] Not an image (${contentType}) for ${url.substring(0, 80)}...`);
      return null;
    }

    const buffer = Buffer.from(await response.arrayBuffer());
    if (buffer.length < 1000) {
      console.error(`[Hasher] Image too small (${buffer.length}b) — likely placeholder`);
      return null;
    }
    ...
    console.error(`[Hasher] Failed to hash ${url.substring(0, 80)}: ${e.message}`);
    return null;
  }
}
